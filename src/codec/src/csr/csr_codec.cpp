...
bool CSR::CBORCodec<ChallengePassword>::encode(zcbor_state_t *state, const ChallengePassword &input)
{
    ...
    case ChallengePassword::Type::TSTR:
        if (!zcbor_tstr_encode_ptr(
                state,
                reinterpret_cast<const char *>(input.bytes.elements),
                input.bytes.len))
...
{
    if (!zcbor_uint32_put(state, input.c509CertificateRequestType))
...
{
    if (!zcbor_uint32_expect(state, output.c509CertificateRequestType))
...
{
    if (!CBORCodec<CSR::TBSCertificateRequest>::encode(state, input.tbsCertificateRequest))
    ...
    if (input.challengePassword.has_value)
    {
        if (!CSR::CBORCodec<ChallengePassword>::encode(state, input.challengePassword.value))
...
{
    if (!CBORCodec<CSR::TBSCertificateRequest>::decode(state, output.tbsCertificateRequest))
    ...
    if (zcbor_nil_expect(state, NULL))
        output.challengePassword.has_value = false;
    else
    {
        if (!CSR::CBORCodec<ChallengePassword>::decode(state, output.challengePassword.value))
            ZCBOR_ERR(CSR_ERR_REQ_DEC_CHALLENGE);

        output.challengePassword.has_value = true;
    }
