...
struct C509Certificate {
	struct TBSCertificate TBSCertificate_m;
};

struct AlgorithmIdentifier_r {
	union {
		int32_t Int;
		struct zcbor_string oid_m;
		struct {
			struct zcbor_string algorithm;
			struct zcbor_string parameters;
		};
	};
	enum {
		AlgorithmIdentifier_int_c,
		AlgorithmIdentifier_oid_m_c,
		AlgorithmIdentifier_algorithm_l_c,
	} AlgorithmIdentifier_choice;
};

struct Attribute_r {
	union {
		struct {
			int32_t attributeTypeInt;
			struct zcbor_string attributeValueText;
		};
		struct {
			struct zcbor_string attributeTypeOID;
			struct zcbor_string attributeValueBytes;
		};
	};
	enum {
		Attribute_attributeTypeInt_l_c,
		Attribute_attributeTypeOID_l_c,
	} Attribute_choice;
};
...
struct Name_r {
	union {
		struct {
			struct Attribute_r Attribute_m[1024];
			size_t Attribute_m_count;
		};
		struct zcbor_string text_m;
		struct zcbor_string bytes_m;
	};
	enum {
		Name_Attribute_m_l_c,
		Name_text_m_c,
		Name_bytes_m_c,
	} Name_choice;
};
...
struct Extension_r {
	union {
		struct {
			int32_t extensionIDInt;
		};
		struct {
			struct zcbor_string extensionIDOID;
			bool critical_present;
			struct zcbor_string extensionValueBytes;
		};
	};
	enum {
		Extension_extensionIDInt_l_c,
		Extension_extensionIDOID_l_c,
	} Extension_choice;
};
...
struct Extensions_r {
	union {
		struct {
			struct Extension_r Extension_m[1024];
			size_t Extension_m_count;
		};
		int32_t Int;
	};
	enum {
		Extensions_Extension_m_l_c,
		Extensions_int_c,
	} Extensions_choice;
};

struct TBSCertificate {
	int32_t c509CertificateType;
	struct zcbor_string certificateSerialNumber;
	struct AlgorithmIdentifier_r issuerSignatureAlgorithm;
	union {
		struct Name_r Name_m;
	};
	enum {
		TBSCertificate_issuer_Name_m_c,
		TBSCertificate_issuer_null_m_c,
	} issuer_choice;
	struct number_r validityNotBefore;
	union {
		struct number_r time_m;
	};
	enum {
		TBSCertificate_validityNotAfter_time_m_c,
		TBSCertificate_validityNotAfter_null_m_c,
	} validityNotAfter_choice;
	struct Name_r subject;
	struct AlgorithmIdentifier_r subjectPublicKeyAlgorithm;
	struct Extensions_r extensions;
};
