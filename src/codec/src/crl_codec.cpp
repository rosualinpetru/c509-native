...
bool CRL::CBORCodec<TBSCertificateRevocationList>::encode(zcbor_state_t *state,
    ...
    if (!zcbor_list_start_encode(state, input.revokedCertificates.size()))
        ZCBOR_ERR(CRL_ERR_TBSCERTREVLIST_ENC_REVOKEDCERTS_START);

    for (size_t i = 0; i < input.revokedCertificates.size(); i++)
        if (!CBORCodec<RevokedCertificate>::encode(state, input.revokedCertificates[i]))
            ZCBOR_ERR(CRL_ERR_TBSCERTREVLIST_ENC_REVOKEDCERT);

    if (!zcbor_list_end_encode(state, input.revokedCertificates.size()))
...
bool CRL::CBORCodec<TBSCertificateRevocationList>::decode(zcbor_state_t *state, TBSCertificateRevocationList &output) {
    ...
    bool r = zcbor_multi_decode(0, CRL_ERR_TBSCERTREVLIST_DEC_REVOKEDCERT, output.revokedCertificates.len_p(),
                                reinterpret_cast<zcbor_decoder_t *>(CBORCodec<RevokedCertificate>::decode), state,
                                output.revokedCertificates.data_p(), sizeof(RevokedCertificate));
